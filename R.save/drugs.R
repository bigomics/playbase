...
compute_drug_enrichment <- function(obj, X, xdrugs, methods = c("GSEA", "cor"),
  ...
  if (length(meta.gmt) == 0) {
    warning("No valid genesets for compute_drug_enrichment!")
    return(NULL)
  }
  ...
  if (length(gg) < 20) {
    warning("Not enough common genes for compute_drug_enrichment!")
    return(NULL)
  }
