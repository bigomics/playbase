...
compute_connectivity_scores <- function(ngs, sigdb, ntop = 1000, contrasts = NULL,
  ...
  if (!is.h5ref) {
    warning("[pgx.computeConnectivityScores] ERROR: must be H5 formatted file\n")
    return(NULL)
  }
  ...
  if (is.null(h5.file)) {
    warning("[pgx.computeConnectivityScores] ERROR: could not H5 file\n")
    return(NULL)
  }
...
correlate_signature_h5_inmemory <- function(FF, h5.file, nsig = 100,
                                            ntop = 1000, nperm = 1000) {
  if (NCOL(FF) == 1 && class(FF) == "numeric") {
    rn <- names(FF)
    FF <- matrix(FF, ncol = 1)
    rownames(FF) <- rn
  }

  if (is.null(rownames(FF))) stop("F must have rownames")
  ## mouse... mouse...
  rownames(FF) <- toupper(rownames(FF))
...
correlate_signature_h5 <- function(fc, h5.file, nsig = 100,
                                   ntop = 1000, nperm = 10000,
