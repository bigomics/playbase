  ...
  if (!is.null(pgx$drugs) && is.null(pgx$drugs$report)) {
    ...
    for (db in drug.db) {
      pgx$drugs[[db]]$report <- cmap.create_report(
        pgx,
        model = llm_model, model2 = NULL, db = db,
        user.prompt = NULL
      )
    }
  }
...
rpt.compile_wgcna_report <- function(obj, report = NULL,
  ...
  if (is.mox) {
    ...
    wgcna.plotMultiDendroAndColors(wgcna$layers,
      show.traits = 1,
    ...
    wgcna.plotDendroAndColors(wgcna,
      show.traits = 1,
      main = "Cluster dendrogram and module colors"
    )
    dev.off()
  ...
  if (is.mox) {
    wgcna.plotModuleTraitHeatmap(
      wgcna$layers,
      multi = TRUE, cluster = TRUE, text = FALSE,
  ...
  df2 <- wgcna.calculateSignificanceScore(wgcna,
    collapse = FALSE,
...
rpt.compile_cmap_report <- function(obj, which.db = 1, report = NULL,
  ...
  div.refs <- c(
    "Subramanian, A., Tamayo, P., Mootha, V. K., .... & Mesirov, J. P. (2005). Gene set enrichment analysis: A knowledge-based approach for interpreting genome-wide expression profiles. PNAS, 102(43), 15545-15550. https://www.pnas.org/doi/10.1073/pnas.0506580102",
...
mofa.compile_markdown_report <- function(mofa, report = NULL,
  ...
  ## ------- tables -------------
  topmodules <- names(rpt$summaries)
  df1 <- mofa.feature_significance(mofa,
    collapse = FALSE,
    sort.by = "score", annot.cols = c("feature", "symbol")
  )
