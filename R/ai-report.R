...
pgx.update_infographics <- function(pgx, llm_model, img_model,
  ...
  if(!is.null(pgx$wgcna) && "wgcna" %in% select) {
    if(force) pgx$wgcna$report$infographic <- NULL    
    pgx$wgcna <- wgcna.init(pgx$wgcna, annot=pgx$genes, llm=llm_model,
      img_model=img_model)
  }
  if(!is.null(pgx$wgcna_mox) && "wgcna" %in% select) {
    if(force) pgx$wgcna_mox$report$infographic <- NULL        
    pgx$wgcna_mox <- wgcna.init(pgx$wgcna_mox, annot=pgx$genes, llm=llm_model,
      img_model=img_model)
  }
  if(!is.null(pgx$drugs) && "cmap" %in% select) {    
    if(force) {
