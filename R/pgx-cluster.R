...
pgx.clusterSamples <- function(pgx,
                               ...
                               replace.orig = TRUE) {
  if (!is.null(X)) {
    message("[pgx.clusterSamples] Using normalized X matrix provided")
  } else if (!is.null(pgx$X)) {
    message("[pgx.clusterSamples] Using normalized X matrix detected in pgx.")
    X <- pgx$X
  } else {
...
pgx.clusterMatrix <- function(X,
  ...
  ## if(datatype %in% c("scRNAseq","scRNA-seq") && cluster.by=="genes") {
  ##   LL <- list(
  ##     X=X,
  ...
  ##     reduce.sd=reduce.sd,
  ##     datatype = datatype,
  ##     center.features = center.features,
  ##     scale.features = scale.features,
  ##     find.clusters = find.clusters,
  ##     svd.gamma = svd.gamma,
  ##     umap.pkg = umap.pkg
  ##   )
  ##   saveRDS(LL, "~/Desktop/MNT/LL-fixumap0.RDS") ## AZ
  ## }
  ...
  if (find.clusters) {
    ...
    idx <- pgx.findLouvainClusters(
      t(X), datatype = datatype, level = 1,
