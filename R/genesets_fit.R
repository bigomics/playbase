...
fit_geneset_contrasts_with_all_methods <- function(gmt, X, Y, G, design, contr.matrix, methods,
  ...
  if ("spearman" %in% methods) {
    ...
    tt <- system.time({
      ...
      ## compute LIMMA
      all.results[["spearman"]] <- fit_geneset_contrasts_with_limma(
  ...
  if ("ssgsea" %in% methods) {
    tt <- system.time({
      zx.ssgsea <- GSVA::gsva(as.matrix(X), gmt[],
        method = "ssgsea",
        parallel.sz = mc.cores, verbose = FALSE
      )
      ...
      all.results[["ssgsea"]] <- fit_geneset_contrasts_with_limma(
  ...
  .fit_this_contrast_with_method <- function(method, k) {
    ...
    if ("ssgsea" %in% method) {
      ...
      tt <- system.time(
        output <- pgx_limma(zx[gs, ], yy, fdr = 1, lfc = 0, ref = ref, trend = LIMMA.TREND, verbose = 0) ## ssgsea
      )
      timings <- rbind(timings, c("ssgsea", tt))
      output <- output[match(names(gmt), rownames(output)), ]
      rownames(output) <- names(gmt)
    ...
    if ("spearman" %in% method && !is.null(zx.rnkcorr)) {
      tt <- system.time(
        output <- pgx_limma(zx.rnkcorr[, ], yy, fdr = 1, lfc = 0, ref = ref, trend = LIMMA.TREND, verbose = 0) ## ssgsea
      )
      timings <- rbind(timings, c("spearman", tt))
      output <- output[match(names(gmt), rownames(output)), ]
      rownames(output) <- names(gmt)
    ...
    if ("gseaPR" %in% method) {
      ...
      tt <- system.time(
        output <- run_gsea_preranked(rnk, gmt,
          fdr = 1.0, do.leading.edge = FALSE,
  ...
  .fit_contrasts_with_method <- function(method) {
    ...
    for (k in 1:ncontrasts) {
      res <- .fit_this_contrast_with_method(method = method, k)
  ...
  m <- "fisher"
  for (m in methods2) {
    res <- .fit_contrasts_with_method(method = m)
...
# gset.fitContrastsWithLIMMA
fit_geneset_contrasts_with_limma <- function(gsetX, contr.matrix, design,
