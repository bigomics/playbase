...
contrasts2pheno <- function(contrasts, samples) {
  ...
  col1 <- apply(samples, 2, function(x) all(colSums(table(x, pheno) != 0, na.rm = TRUE) == 1))
  row1 <- apply(samples, 2, function(x) all(rowSums(table(x, pheno) != 0, na.rm = TRUE) == 1))
  if (any(col1 & row1)) {
...
makeContrastFromSamples <- function(samples, ref = NULL) {
  ct <- pgx.makeAutoContrasts(
    samples,
    mingrp = 2, slen = 20, ref = ref, max.level = 99,
    fix.degenerate = FALSE, skip.hidden = TRUE
  )
