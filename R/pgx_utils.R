...
.probe2symbol <- function(probes, type=NULL, org="human", keep.na=FALSE)
{
    ...
    if(!type %in% c("ensembl","ensemblTRANS","unigene","refseq",
    ...
    if(type=="symbol") {
        ...
        if(org=="human") {
            symbol0 <- AnnotationDbi::mapIds(org.Hs.eg.db::org.Hs.eg.db,
                                             probes, 'SYMBOL', toupper(type))
        }
        if(org=="mouse") {
            symbol0 <- AnnotationDbi::mapIds(org.Mm.eg.db::org.Mm.eg.db,
                                             probes, 'SYMBOL', toupper(type))
        }
    }
