...
# =============================================================================
# AI-report builders (cp from v03 board.drugconnectivity/R/ai.report/
# ai_report_data.R; DRUGCONNECTIVITY_PROMPTS_DIR replaced by .drugs_prompt_path)
# =============================================================================
...
drugconnectivity_build_methods <- function(pgx, method) {
  template <- omicsai::omicsai_load_template(
    .drugs_prompt_path("drugconnectivity_methods.md")
  )
...
#' Generate a drug-connectivity AI report (phase-2 static path).
#'
...
#' issues one LLM call per DB, concatenates into a single report.
#'
#' @param slice pgx$drugs (named list keyed by drug DB).
#' @param pgx full pgx object.
#' @param ai resolved `ai` list.
#' Sanitize a drug-DB key to a slot-safe suffix.
#' "L1000/activity" -> "L1000_activity"; "CTRPv2/sensitivity" -> "CTRPv2_sensitivity".
#' @keywords internal
...
#' @export
drugs.create_report <- function(slice, pgx, ai) {
  if (!requireNamespace("omicsai", quietly = TRUE)) {
    stop("omicsai package required for AI report generation", call. = FALSE)
  }
