...
gsea_leading_edge_analysis <- function(output.dir, ntop = 100,
                                       gsea.program = GSEA.JAR, xmx = 10) {
  ## clean old results
...
run_gsea_preranked <- function(rnk, gmt, output.dir = NULL, fdr = 0.25,
  ...
  rr <- dir(d1, pattern = "^gsea_report_for_.*xls$")
  if (length(rr) == 0) {
    warning("Error: could not find report files. exiting.\n")
    return(1)
  }
  r1 <- read.csv(paste(d1, rr[1], sep = "/"),
    sep = "\t",
  ...
  if (sum(!(rownames(res) %in% names(gmt.origname))) > 0) {
    warning("warning: could not map some altnames back\n")
  }

  ## fill in missing value with closest neighbour
  na.sum <- sum(is.na(res$NES))
  if (na.sum > 0) {
    warning("WARNING:: GSEA returned some missing values!\n")
