...
queryTileDB <- function(tiledb_path, genes, samples = NULL, as_matrix = TRUE,
  ...
  ## Push the gene (and optional sample) selection down to TileDB via dimension
  ## ranges, so only the matching cells are read instead of scanning the whole
  ## array into R and subsetting afterwards. gene and sample are the array
  ## dimensions, so each requested value becomes a [x, x] point-range.
  arr <- tiledb::tiledb_array(tiledb_path, return_as = "data.frame")
  ranges <- list(gene = cbind(genes, genes))
  if (!is.null(samples) && length(samples) > 0) {
    ranges[["sample"]] <- cbind(samples, samples)
  }
  tiledb::selected_ranges(arr) <- ranges
  df <- arr[]
  ...
  ## Vectorized fill: map each row's (gene, sample) to matrix positions in one
  ## shot instead of an O(nrow) loop with per-row %in% checks.
  gi <- match(df$gene, genes)
  si <- match(df$sample, sample_names)
  keep <- !is.na(gi) & !is.na(si)
  mat[cbind(gi[keep], si[keep])] <- df$value[keep]
...
listGenesTileDB <- function(tiledb_path) {
  ...
  ## Exclude features without a usable identifier ("-" and "." placeholders). New
...
tiledb.prepareData <- function(pgx) {
  ...
  if (length(gene_ids) > 0) {
    ## counts and zscores come from pgx$counts and pgx$X, which can carry
    ## different feature sets, so align zscores to the counts rows by gene id
    ## (tiledb.writeData assumes they are row-aligned). Unmatched genes get NA.
    zscores <- zscores[match(gene_ids, rownames(zscores)), , drop = FALSE]

    ## Drop features without a usable identifier: NA names and "-"/"." placeholder
    ## rows left by ortholog mapping; these should never enter the database.
    keep <- !is.na(gene_ids) & !grepl("^[.-]", gene_ids)
    counts <- counts[keep, , drop = FALSE]
    zscores <- zscores[keep, , drop = FALSE]
  }
