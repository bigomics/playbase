...
gx.limma <- function(X,
                     ...
                     ref = c(
                       ...
                       "non", "no", "not", "neg", "negative", "n", "ref", "veh", "vehicle",
  ...
  if (length(levels) != 2 ) {
    stop("[gx.limma] ERROR: only two class comparisons. Please use gx.limmaF for N>2\n\n")
    return(NULL)
  }
...
#' F-test with limma for testing difference in mean of more than two classes.
#'
...
gx.limmaF <- function(X, pheno, B = NULL, fdr = 0.05, compute.means = TRUE, lfc = 0.20,
                      ...
                      ref = c(
                        ...
                        "non", "no", "not", "n", "neg", "negative", "ref", "veh", "vehicle",
                        "wt", "wildtype", "untreated", "normal", "false", "healthy"
                      ),
  ...
  if (length(levels) < 2) {
    stop("gx.limmaF::fatal error:need at least two classes")
    return(NULL)
  }
