...
pgx.scatterPlot <- function(pgx, pheno = NULL, gene = NULL,
  ...
  if (!is.null(contrast)) {
    ...
    plt <- pgx.scatterPlotXY(
      pos, var,
      plotlib = plotlib, ## title=contrast,
      xlab = colnames(pos)[1], ylab = colnames(pos)[2],
      tooltip = tooltip,
      ...
    )
...
pgx.scatterPlotXY.BASE <- function(pos, var = NULL, type = NULL, col = NULL, title = "",
  ...
  if (type == "factor") {
    ...
    if (is.null(col) && nz > 2) {
      col1 <- c(
        RColorBrewer::brewer.pal(8, "Set1"), # omics_pal_d("muted")(8),
  ...
  if (type == "numeric") {
    ...
    if (legend.pos != "none" && legend) {
      ...
      legend(legend.pos,
        cex = 0.8 * cex.legend, ## text.width=2,
