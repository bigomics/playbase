...
test_that("Z-scores are computed independently per dataset", {
  ...
  samples_a <- data.frame(group = rep("A", 3), row.names = paste0("a_sample_", 1:3))
  X_a <- log2(counts_a + 1)
  ...
  samples_b <- data.frame(group = rep("B", 3), row.names = paste0("b_sample_", 1:3))
  X_b <- log2(counts_b + 1)
...
test_that("Z-scores handle NA values in counts", {
  ...
  samples <- data.frame(group = rep("test", 4), row.names = paste0("s", 1:4))
  X <- log2(counts + 1)
  pgx <- list(counts = counts, X = X, genes = genes, samples = samples)
  save_mock_pgx(pgx, file.path(pgx_dir, "test.pgx"))
...
test_that("Z-scores work with tiledb.addDataset", {
  ...
  samples1 <- data.frame(group = rep("A", 3), row.names = paste0("s1_", 1:3))
  X1 <- log2(counts1 + 1)
  ...
  samples2 <- data.frame(group = rep("B", 4), row.names = paste0("s2_", 1:4))
  X2 <- log2(counts2 + 1)
