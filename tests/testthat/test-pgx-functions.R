...
test_that("trimsame dispatches to trimsame0 or trimsame.ends", {
  ...
  expect_equal(
    playbase::trimsame(c("a b c", "a b d"), ends = FALSE),
    c("c", "d")
  )
  # ends = TRUE (default) trims both ends, non-space split
  expect_equal(
    playbase::trimsame(c("grp_a_b_c_end", "grp_z_end"), split = "_", ends = TRUE),
    c("a_b_c", "z")
  )
  # all-NA and all-empty short-circuit and return input unchanged
  expect_equal(playbase::trimsame(c(NA_character_, NA_character_)), c(NA_character_, NA_character_))
  expect_equal(playbase::trimsame(c("", "")), c("", ""))
})

#' Test for trimsame.ends
test_that("trimsame.ends trims common prefix and suffix", {
  expect_equal(
    unname(playbase::trimsame.ends(c("pre mid post", "pre other post"))),
    c("mid", "other")
  )
  # non-space split must round-trip correctly (collapse uses split, not " ")
  expect_equal(
    playbase::trimsame.ends(c("grp_a_b_c_end", "grp_z_end"), split = "_"),
    c("a_b_c", "z")
  )
})
...
test_that("trimsame0 trims common leading/trailing tokens without erroring", {
  ...
  expect_equal(playbase::trimsame0(c("a b c", "a b d")), c("c", "d"))

  # common prefix only
  expect_equal(playbase::trimsame0(c("a b c", "a b d e")), c("c", "d e"))

  # common suffix only
  expect_equal(playbase::trimsame0(c("a b c x", "z x")), c("a b c", "z"))

  # common prefix and suffix
  expect_equal(
    unname(playbase::trimsame0(c("pre mid post", "pre other post"))),
    c("mid", "other")
  )

  # no common leading/trailing token at all
  expect_equal(playbase::trimsame0(c("x y", "z w")), c("x y", "z w"))

  # non-space split
  expect_equal(playbase::trimsame0(c("a_b_c", "a_b_d"), split = "_"), c("c", "d"))

  # all-NA and all-empty short-circuit and return input unchanged
  expect_equal(playbase::trimsame0(c(NA_character_, NA_character_)), c(NA_character_, NA_character_))
  expect_equal(playbase::trimsame0(c("", "")), c("", ""))

  # trimming consumes every token (strings identical) -> falls back to
  # the first original token for each element
  expect_equal(playbase::trimsame0(c("a b", "a b")), c("a", "a"))
})
...
test_that("trimsame0 preserves input length and never errors on ragged input", {
  ...
  expect_equal(playbase::trimsame0(c("x y c", "z w c")), c("x y", "z w"))
  expect_length(playbase::trimsame0(c("x y c", "z w c")), 2L)

  # one element emptying before the others used to error with
  # "comparison of these types is not implemented"
  expect_equal(playbase::trimsame0(c("p a x", "p b x", "p x")), c("a", "b", ""))
  ...
  for (input in list(
    ...
    expect_length(playbase::trimsame0(input), length(input))
    expect_length(playbase::trimsame.ends(input), length(input))
  }
})
