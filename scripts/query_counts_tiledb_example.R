...
# Get database info
cat("=== Database Info ===\n")
playbase::infoTileDB(tiledb_path)
...
if ("TP53" %in% genes) {
  counts_tp53 <- playbase::queryTileDB(tiledb_path, genes = "TP53")
  ...
  counts_first <- playbase::queryTileDB(tiledb_path, genes = genes[1])
  cat("Gene:", genes[1], "\n")
...
counts_multi <- playbase::queryTileDB(tiledb_path, genes = query_genes)
...
df <- playbase::queryTileDB(tiledb_path, genes = query_genes[1], as_matrix = FALSE)
print(head(df, 10))
